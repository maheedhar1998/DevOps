...
module "subnet" {
  ...
  name = "Main 10.12.1.0/24 ${module.VPC.vpc_id}"
}

module "subnet_2" {
  source = "../../modules/subnet"
  vpc_id = module.VPC.vpc_id
  subnet_cidr_block = "10.12.2.0/24"
  name = "Main 10.12.2.0/24 ${module.VPC.vpc_id}"
}

module "ec2" {
  source = "../../modules/ec2/red_hat"
  subnet_id = module.subnet_1.subnet_id
  name = "My WebServer Prod"
...
module "load_balancer" {
  ...
  name = "Apllication-Load-Balancer-Prod"
  internal = false
  lb_type = "application"
